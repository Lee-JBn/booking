...
public class SeatController3 implements Initializable {

	private static final String MaterialIconView = null;
	...
	private void nextBtnAction(ActionEvent event) {
		...
		try {
			...
			pstmt = con.prepareStatement("update reservationmember set startseat = ?, serial = ? where phone = ?");
	...
	private void returnBtnAddAction(ActionEvent event) {
		...
		try {
			...
			System.out.println(i + "명의 예약을 취소했습니다.");
		} catch(SQLException e) {
		} finally {
			try { if(pstmt != null) pstmt.close();} catch(SQLException e) {}
			try { if(con != null) con.close();} catch(SQLException e) {}
